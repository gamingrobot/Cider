...
from . CiderProperties import CiderPropertyGroup
...
class CiderMaterial(bpy.types.PropertyGroup):
    ...
    def poll_tree(self, object):
        return object.bl_idname == 'CiderTree' and (object.graph_type == self.material_type or self.material_type == '')
    ...
    shader_source : bpy.props.StringProperty(name="Shader Source", subtype='FILE_PATH', update=update_source,
        set=cider_path_setter('shader_source'), get=cider_path_getter('shader_source'),
        options={'LIBRARY_EDITABLE'}, override={'LIBRARY_OVERRIDABLE'})
    ...
    parameters : bpy.props.PointerProperty(type=CiderPropertyGroup, name="Shader Parameters",
        options={'LIBRARY_EDITABLE'}, override={'LIBRARY_OVERRIDABLE'})
    ...
    def draw_ui(self, layout, extension, material_parameters):
        ...
        layout.prop_search(self, 'material_type', bpy.context.scene.world.cider, 'material_types')
        row = layout.row()
        ...
        if self.shader_nodes:
            row.operator('wm.cider_callback', text='', icon='DUPLICATE').callback.set(nodes_add_or_duplicate, 'Duplicate')
            # from BlenderMalt.MaltNodes.MaltNodeTree import set_node_tree
            # row.operator('wm.cider_callback', text = '', icon = 'GREASEPENCIL').callback.set(
            #     lambda: set_node_tree(bpy.context, self.shader_nodes)
            # )
        else:
            row.operator('wm.cider_callback', text='New', icon='ADD').callback.set(nodes_add_or_duplicate, 'New')
        ...
        if self.compiler_error != '':
            layout.operator("wm.cider_print_error", icon='ERROR').message = self.compiler_error
            box = layout.box()
    ...
    def draw(self, context):
        ...
        if ob:
            ...
            def add_or_duplicate():
                ...
                material.cider.material_type = 'Mesh'
                if slot:
        ...
        if context.material:
            context.material.cider.draw_ui(layout, '.mesh.glsl', context.material.cider_parameters)
...
def track_shader_changes(force_update=False, async_compilation=True):
    from Cider import CiderPipeline
    if CiderPipeline.is_cider_active() == False and force_update == False:
        return 1
    ...
    try:
        ...
        if len(needs_update) > 0:
            compiled_materials = CiderPipeline.get_bridge().compile_materials(needs_update, async_compilation=async_compilation)
        else:
            compiled_materials = CiderPipeline.get_bridge().receive_async_compilation_materials()
        ...
        if len(compiled_materials) > 0:
            ...
            for material in bpy.data.materials:
                ...
                if path in compiled_materials.keys():
                    material.cider.compiler_error = compiled_materials[path].compiler_error
                    #TODO: Use parent parameters as defaults for materials with nodes
                    material.cider.parameters.setup(compiled_materials[path].parameters)
